...
function capitalA(s){
    ...
    let resNum=regexNum.test(s);
    let resA=regexA.test(s); 
...
function ioEmail(email){
    let regexIo = /.io$/g;
    let resIo=regexIo.test(email);

    let regexSections=/\W/gi;
    let resSections =email.match(regexSections);
    if(resIo && resSections.length==2)
    return true;
    else
    return false; 
}
